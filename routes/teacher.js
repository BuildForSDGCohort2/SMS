...
router.post('/percentage/:id',async(req,res)=>{
  var decode = jwt.verify(req.headers['authorization'], key)
  Percentage.find({school_id:decode.school_id,student_id:req.params.id,term:req.body.term})
    .then(result=>{
      if(result){
        Percentage.findOneAndUpdate({school_id:decode.school_id,student_id:req.params.id,term:req.body.term},{
          $set: {
        ...
        .then(() => {
          res.json('Percentage Updated')
        })
      }
      else{
        var newPercentage = new Percentage({
          percentage:req.body.percentage,
          total:req.body.total,
          test:req.body.test,
          exam:req.body.exam,
          ...
          term:req.body.term,
          index:req.body.term==='1stTerm' ? 1 : req.body.term==='2ndTerm' ? 2 : 3
        })
        newPercentage.save()
          .then(() => {
            res.json('Percentage Added')
          })
      }
    })
...
router.get('/percentage/:student_id', (req,res)=>{
  var decode = jwt.verify(req.headers['authorization'],key)
  Percentage.find({student_id:req.params.student_id,clas:decode.clas,school_id:decode.school_id})
  .then(percentage => res.json(percentage))
  .catch(err => res.status(400).json('Error: ' + err))
})
...
router.get('/teacher/:clas', (req,res)=>{
  Teacher.findOne({clas:req.params.clas, status:'registered'})
